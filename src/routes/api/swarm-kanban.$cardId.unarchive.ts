...
import { unarchiveKanbanCard } from '../../server/kanban-backend'
import { isLifecycleV2Enabled } from '../../lib/feature-flags'

export const Route = createFileRoute('/api/swarm-kanban/$cardId/unarchive')({
  server: {
    handlers: {
      POST: async ({ request, params }) => {
        ...
        try {
          const result = await unarchiveKanbanCard({ cardId, actor })
          if (!result) {
