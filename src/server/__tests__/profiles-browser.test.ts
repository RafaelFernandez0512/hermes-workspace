...
describe('profiles-browser', () => {
  ...
  describe('setProfileTargetId', () => {
    it('writes workspaceTargetId to the profile config', async () => {
      const root = path.join('/home/testuser', '.hermes')
      const configPath = path.join(root, 'config.yaml')
      ...
      readFileSync.mockImplementation((p: string) => {
        if (p === configPath) return 'model: gpt-4\n'
        return ''
      })

      const mod = await loadMod()
      mod.setProfileTargetId('default', 'pc-gaming')

      const writtenCall = writeFileSync.mock.calls.find((call) =>
        (call[0] as string).endsWith('config.yaml'),
      )
      expect(writtenCall).toBeDefined()
      expect(writtenCall![1] as string).toContain('workspaceTargetId: pc-gaming')
    })

    it('removes workspaceTargetId when called with undefined', async () => {
      const root = path.join('/home/testuser', '.hermes')
      const configPath = path.join(root, 'config.yaml')
      ...
      readFileSync.mockImplementation((p: string) => {
        if (p === configPath) return 'model: gpt-4\nworkspaceTargetId: pc-gaming\n'
        return ''
      })

      const mod = await loadMod()
      mod.setProfileTargetId('default', undefined)

      const writtenCall = writeFileSync.mock.calls.find((call) =>
        (call[0] as string).endsWith('config.yaml'),
      )
      expect(writtenCall).toBeDefined()
      expect(writtenCall![1] as string).not.toContain('workspaceTargetId:')
    })
  })
})
